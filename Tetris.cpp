...
void Tetris::inicialitzaJoc(int mode, const string& fitxerInicial, const string& fitxerFigures,
	const string& fitxerMoviments)
{
}
void Tetris::mostraPuntuacions() const
{
	string msg = "Files Fetes: " + to_string(m_partida.getNivellActual()) + ", Puntuacio: " + to_string(m_partida.getPuntuacio());
...
bool Tetris::juga(int mode, double deltaTime)
{
	if (!m_partida.getPartidaAcabada())
	{
		...
		string msg = "Punts: " + to_string(m_partida.getPuntuacio()) + ", Nivell: " +
			to_string(m_partida.getNivellActual());
		GraphicManager::getInstance()->drawFont(FONT_WHITE_30,
			POS_X_TAULER, POS_Y_TAULER - 50, 1.0, msg);
	}
	else
	{
		...
		GraphicManager::getInstance()->drawFont(FONT_WHITE_30,
			SCREEN_SIZE_X / 2 - 180, SCREEN_SIZE_Y / 2 - 50, 2.0, gameOver);
	}
